...
public class WolfInfoScreen extends BaseOwoScreen<FlowLayout> {
    ...
    protected void build(FlowLayout rootComponent) {
        ...
        rootComponent.child(Containers.draggable(Sizing.content(), Sizing.content(),
                Containers.verticalFlow(Sizing.content(), Sizing.content())
                        .child(Components.label(Text.literal("Preview")).horizontalTextAlignment(HorizontalAlignment.CENTER))
                        .child(Components.entity(Sizing.fixed(256), target).scaleToFit(true).allowMouseRotation(true).margins(Insets.of(5)))
                        ...
                        .child(Components.label(Text.literal("Health - " + this.target.getHealth() + "/" + this.target.getMaxHealth()))).padding(Insets.of(5)).surface(Surface.DARK_PANEL).verticalAlignment(VerticalAlignment.CENTER).horizontalAlignment(HorizontalAlignment.CENTER)
        ));
    }
}
